...
<body>
  <div class="container">
    ...
    {performance_section}
    {results_section}
    <div class="performance-tracker" style="margin-top: 1.5rem;">
      <h2 style="color:var(--accent);font-size:1.2rem;margin-bottom:1rem;">Model Predictions</h2>
...
def render_strategy_section(strategy) -> str:
    ...
    def fmt_prob(v):
        try:
            ...
            return "N/A"
        # display formatting only: some rows carry prob as a percent (92.2),
        # others as a fraction (0.922)
        if v > 1:
            return f"{v:.0f}%"
        return f"{v:.0%}"
    ...
    def action_pill(text, bg="var(--accent)"):
        if not text:
            return ""
        return (
            f'<div style="margin:0.6rem 0 0.2rem;">'
            f'<span style="display:inline-block;padding:0.35rem 0.85rem;'
            f'border-radius:8px;font-size:0.9rem;font-weight:800;letter-spacing:0.01em;'
    ...
    if "AI analysis unavailable" in gw_summary:
        parts.append(
            '<div style="background:rgba(251,191,36,0.08);'
            'border:1px solid rgba(251,191,36,0.3);border-radius:8px;'
            'padding:0.5rem 1rem;font-size:0.85rem;color:#fbbf24;'
            'margin-bottom:1rem;">'
            'AI analysis unavailable \u2014 showing model signals only</div>'
        )
    if gw_summary:
        parts.append(
    ...
    rest_para = (strategy.get("rest_of_card_paragraph") or "").strip()
    if rest_para:
        parts.append(
            '<div style="font-size:0.75rem;color:var(--muted);'
            'margin:1rem 0 0.4rem;text-transform:uppercase;letter-spacing:0.05em;">'
            "Pundit&#39;s View on the Rest of the Card</div>"
        )
        parts.append(
            '<div style="background:rgba(17,24,39,0.4);'
            'border:1px solid var(--panel-border);border-radius:12px;'
            'padding:1rem 1.2rem;font-style:italic;color:var(--text);'
            f'font-size:0.95rem;">{esc(rest_para)}</div>'
        )

    # betting_category (from match_context.json, refreshed each run) is the
    # source of truth for these sections - NOT the possibly-stale is_strong_fade.
    fixtures_full = strategy.get("fixtures_full") or []

    def _by_category(cat, fallback_key):
        rows = [fx for fx in fixtures_full if fx.get("betting_category") == cat]
        if not rows:
            rows = strategy.get(fallback_key) or []
        return rows

    strong_fades = _by_category("strong_fade", "strong_fades")
    if strong_fades:
        parts.append(
            '<h3 style="color:#fbbf24;font-size:0.9rem;'
            'margin:1rem 0 0.6rem;">Strong Fades</h3>'
        )
        parts += [
            card(fx, border="#fbbf24", shadow="rgba(251,191,36,0.15)", show_action=True)
            for fx in strong_fades
        ]

    double_chances = _by_category("double_chance", "double_chances")
    if double_chances:
        parts.append(
            '<h3 style="color:#a78bfa;font-size:0.9rem;'
            'margin:1rem 0 0.6rem;">Double Chance</h3>'
        )
        parts += [
            card(fx, border="#a78bfa", shadow="rgba(167,139,250,0.15)", show_action=True)
            for fx in double_chances
        ]
